...
$q = trim((string) ($_GET['q'] ?? '')); $tag = trim((string) ($_GET['tag'] ?? ''));
$category = $page['category'] ?? null;   // subsección /blog/<categoria>/ (la pone el enrutador); si no, el filtro ?cat= de siempre
$cat = $category ? '' : trim((string) ($_GET['cat'] ?? ''));
$posts = $category ? cms_items_in_category('posts', $category['slug']) : cms_items('posts');
...
$base = $category ? $category['url'] : $blog;   // las páginas del paginador se quedan en la subsección
$catUrl = fn(array $p) => ($s = cms_item_category('posts', $p)) !== '' ? cms_url('cat:posts', $lang, $s) : $blog . '?cat=' . rawurlencode((string) cms_f($p, 'category', $lang)); ?>
<section class="wrap">
  <h1><?= cms_e($category ? $category['label_text'] : $t('posts_title', 'Blog')) ?></h1>
<?php if ($category): ?><p class="muted"><?php if (($d = cms_category_text($category, 'desc', $lang)) !== ''): ?><?= cms_e($d) ?> · <?php endif; ?><a href="<?= $blog ?>"><?= cms_e($t('blog_filter_clear', 'ver todo')) ?></a></p><?php endif; ?>
...
<?php foreach ($posts as $p): $u = cms_url('item:posts', $lang, $p['slug']); ?>
    <article class="card">
      ...
      <div class="card-b">
        <small><?php if (cms_f($p, 'category', $lang) !== ''): ?><a href="<?= cms_e($catUrl($p)) ?>"><?= cms_e(cms_f($p, 'category', $lang)) ?></a> · <?php endif; ?><?= cms_date($p['date'] ?? '', $lang) ?></small>
...
<?php if ($pages > 1): ?>
  <nav class="pager"><?php for ($i = 1; $i <= $pages; $i++): $qs = http_build_query(array_filter(['q' => $q, 'tag' => $tag, 'cat' => $cat, 'pg' => $i > 1 ? $i : null])); ?><a href="<?= $base . ($qs ? '?' . $qs : '') ?>"<?= $i === $pg ? ' class="on"' : '' ?>><?= $i ?></a><?php endfor; ?></nav>
<?php endif; ?>
</section>
