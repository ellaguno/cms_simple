...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=<?= rawurlencode($font) ?>:wght@400;500;600;700;800&display=swap">
<link rel="stylesheet" href="<?= cms_e($aos['css'][0]) ?>">
<link rel="stylesheet" href="<?= cms_asset('css/lienzo.css') ?>?v=<?= CMS_VERSION ?>">
<style>:root{--lz-font:"<?= cms_e($font) ?>",system-ui,sans-serif;--lz-primary:<?= cms_e($primary) ?>;--lz-accent:<?= cms_e($accent) ?>;--cms-accent:<?= cms_e($accent) ?>;--cms-text:<?= cms_e($primary) ?>}</style>
</head>
